...
func generate_response(prompt: String, params: Dictionary = {}, use_tools: bool = false) -> Dictionary:
	...
	while retry_count < max_retries:
		...
		if response.has("Error") and "Unexpected response format" in response["Error"]:
			retry_count += 1
			if debug: print("LLM: Unexpected response format. Retrying... (Attempt ", retry_count, "/", max_retries, ")")
			await get_tree().create_timer(1.0).timeout  # Wait for 1 second before retrying
			_reinitialize_api()
		else:
			break
	
	if retry_count == max_retries:
		push_error("Max retries reached. Unable to get a valid response from the API.")
	...
	# (Tool calling loop and other existing logic)
...
func _reinitialize_api():
	if debug: print("LLM: Reinitializing API")
	if api:
		remove_child(api)
		api.queue_free()
	_initialize_api()
